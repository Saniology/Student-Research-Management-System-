...
const url = process.env.SPMS_RENDER_URL || 'http://127.0.0.1:5500/';
...
async function main() {
  ...
  if (!serverOk) {
    const message = `local app is not reachable at ${url}; start it with python3 -m http.server 5500 --bind 0.0.0.0`;
    if (required) fail(message);
    else warn(message);
    finish();
    return;
