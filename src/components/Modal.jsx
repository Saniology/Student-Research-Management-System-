...
export function Modal({ open, onClose, title, eyebrow, children, wide = false, id, variant = 'default', className = '' }) {
  useEffect(() => {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" onMouseDown={event => event.target === event.currentTarget && onClose()}>
    <div className={`modal-shell ${wide ? 'modal-wide' : ''} ${variant === 'auth' ? 'auth-modal-shell' : ''} ${className}`} id={id}>
