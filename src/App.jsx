...
function StudentWorkspace({ profile, session, preview, onToast, onProfileUpdate }) {
  ...
  const [projectReviews, setProjectReviews] = useState(preview && revisionPreview ? [{ id: 'preview-review', action: 'revision_requested', comment: 'Please clarify the methodology section and add the missing references before resubmitting.', annotations: [{ type: 'arrow', x1: 24, y1: 42, x2: 62, y2: 28 }], version_number: 1, created_at: new Date().toISOString() }] : []);
...
function ProjectFeedbackPanel({ versions = [], reviews = [] }) {
  const revision = reviews.find(item => item.action === 'revision_requested');
  const latest = versions[versions.length - 1];
  return <section className="surface project-feedback-panel">
    <div className="surface-head"><div><p className="eyebrow">Review feedback</p><h2>Corrections and version history</h2><p>Each resubmission is stored as a new version. Earlier uploads cannot be replaced or restored.</p></div><span className="tag"><RefreshCw size={12} />{versions.length ? 'Version ' + versions[versions.length - 1].version_number : 'Version tracking'}</span></div>
    {revision?.comment && <div className="revision-banner"><PencilLine size={16} /><div><strong>Supervisor correction note</strong><span>{revision.comment}</span></div></div>}
    {revision?.annotations?.length > 0 && <><p className="helper"><strong>{revision.annotations.length} visual correction mark{revision.annotations.length === 1 ? '' : 's'}</strong> were added to the private PDF review.</p><ReviewAnnotationPreview annotations={revision.annotations} /></>}
    {versions.length ? <div className="version-history">{versions.map(version => <div className="version-history-item" key={version.id || version.version_number}><div><strong>Version {version.version_number}</strong><span>{version.source === 'resubmission' ? 'Resubmission for another review' : 'Initial submission'} · {version.file_name || 'PDF upload'}</span></div><div><span className="tag">{version.source === 'resubmission' ? 'RESUBMITTED' : 'ORIGINAL'}</span><small>{displayDate(version.created_at)}</small></div></div>)}</div> : <EmptyState icon={FileText} title="Version history will appear here" copy="Your first successful submission becomes version 1." />}
    <details className="student-submission-preview"><summary><Eye size={14} />View latest submission</summary><PdfPreview path={latest?.file_path} /></details>
  </section>;
}
