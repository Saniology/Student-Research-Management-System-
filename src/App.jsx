...
function PasswordField({ id, label, value, onChange, placeholder, minLength, required = true, autoComplete = 'new-password' }) {
  const [visible, setVisible] = useState(false);
  const toggleLabel = `${visible ? 'Hide' : 'Show'} ${label.toLowerCase()}`;
  return <div className="field password-field">
    <label htmlFor={id}>{label}</label>
    <div className="password-input-wrap"><input id={id} type={visible ? 'text' : 'password'} minLength={minLength} required={required} value={value} onChange={onChange} placeholder={placeholder} autoComplete={autoComplete} /><button className="password-toggle" type="button" onClick={() => setVisible(current => !current)} aria-label={toggleLabel} title={toggleLabel}>{visible ? <EyeOff size={16} /> : <Eye size={16} />}</button></div>
  </div>;
}
...
function workspaceSectionTarget(role, label) {
  const targets = {
    student: { Overview: 'student-overview', Submission: 'student-submission', Payments: 'student-payments', Receipt: 'student-receipt', Profile: 'student-profile' },
    teacher: { Overview: 'teacher-overview', 'Assigned projects': 'teacher-projects', 'Assigned students': 'teacher-students', 'Review history': 'teacher-history', 'Supervisor profile': 'teacher-profile' },
...
function ProfileContactForm({ profile, session, preview, onProfileUpdate, onToast, onDone }) {
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ email: profile?.email || '', phone: profile?.phone || '' });
  useEffect(() => setForm({ email: profile?.email || '', phone: profile?.phone || '' }), [profile]);
  const save = async event => {
    event.preventDefault();
    const email = form.email.trim().toLowerCase();
    const phone = form.phone.trim();
  ...
  return <form className="form-grid" onSubmit={save}>
    ...
    <div className="field full"><label htmlFor="profile-email">Email address</label><input id="profile-email" type="email" required value={form.email} onChange={event => setForm({ ...form, email: event.target.value })} /></div>
    <div className="field full"><label htmlFor="profile-phone">Phone number</label><input id="profile-phone" type="tel" value={form.phone} onChange={event => setForm({ ...form, phone: event.target.value })} placeholder="0803 000 0000" /></div>
    <div className="modal-actions full"><button className="button button-ghost" type="button" disabled={saving} onClick={onDone}>Cancel</button><button className="button button-primary" disabled={saving}>{saving ? 'Saving...' : 'Save contact info'}</button></div>
  </form>;
}

function ProfileInformationModal({ profile, session, preview, onProfileUpdate, onToast }) {
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState(false);
  const roleLabel = profile?.role === 'teacher' ? 'Supervisor' : 'Student';
  const close = () => { setEditing(false); setOpen(false); };
  return <>
    <div className="sidebar-profile">
      <button className={`sidebar-profile-button ${open ? 'active' : ''}`} type="button" onClick={() => setOpen(true)} aria-label={`Open ${roleLabel.toLowerCase()} profile`}>
        <ProfileAvatar name={profile?.full_name || roleLabel} src={profile?.avatar_url} />
        <span><strong>{profile?.full_name || roleLabel}</strong><small>View profile</small></span>
        <UserCircle size={17} />
      </button>
    </div>
    <Modal open={open} onClose={close} eyebrow={editing ? 'Profile settings' : `${roleLabel} profile`} title={editing ? 'Edit contact info' : 'Your profile'}>
      {editing ? <ProfileContactForm profile={profile} session={session} preview={preview} onProfileUpdate={onProfileUpdate} onToast={onToast} onDone={() => setEditing(false)} /> : <div className="profile-modal-content">
        <div className="profile-modal-identity"><ProfileAvatar name={profile?.full_name || roleLabel} src={profile?.avatar_url} size="avatar-medium" /><div><p className="eyebrow">Official account</p><h3>{profile?.full_name || roleLabel}</h3><span className="tag"><LockKeyhole size={12} />Name and identity verified</span></div></div>
        <div className="profile-modal-details"><div><small>Department</small><strong>{profile?.department || 'Department not assigned'}</strong></div>{profile?.matric && <div><small>Matric number</small><strong>{profile.matric}</strong></div>}<ContactDetails person={profile} roleLabel={`${roleLabel} account`} /></div>
        <div className="modal-actions"><button className="button button-ghost" type="button" onClick={close}>Close</button><button className="button button-primary" type="button" onClick={() => setEditing(true)}><PencilLine size={15} />Edit contact info</button></div>
      </div>}
    </Modal>
  </>;
}
...
function StudentWorkspace({ profile, session, preview, onToast, onProfileUpdate }) {
  ...
  const [projectVersions, setProjectVersions] = useState(preview ? [{ version_number: 1, source: 'initial', file_name: 'e-voting-system.pdf', title: 'Web-Based E-Voting System', created_at: new Date().toISOString() }, ...(revisionPreview ? [{ version_number: 2, source: 'resubmission', file_name: 'e-voting-system-corrected.pdf', title: 'Web-Based E-Voting System', created_at: new Date().toISOString(), change_summary: 'Corrected methodology section after supervisor feedback.' }] : [])] : []);
  const [projectReviews, setProjectReviews] = useState(preview && revisionPreview ? [{ id: 'preview-review', action: 'revision_requested', comment: 'Please clarify the methodology section and add the missing references before resubmitting.', annotations: [{ id: 'preview-highlight', type: 'text', annotation_type: 'highlight', position: { pageNumber: 1, boundingRect: { left: 18, top: 30, width: 22, height: 3 }, rects: [] }, content: { text: 'methodology section' }, comment: 'Clarify the sample size and research instrument here.' }, { id: 'preview-circle', type: 'area', annotation_type: 'circle', position: { pageNumber: 1, boundingRect: { left: 48, top: 52, width: 16, height: 9 }, rects: [] }, comment: 'Add the missing references.' }, { type: 'arrow', x1: 24, y1: 42, x2: 62, y2: 28 }], version_number: 1, created_at: new Date().toISOString() }] : []);
  ...
  const [activeSection, setActiveSection] = useState(revisionPreview ? 'student-submission' : previewAction === 'show_receipt' ? 'student-receipt' : 'student-overview');
  const pendingVerificationKey = session?.user?.id ? `spms:pending-clearance:${session.user.id}` : '';
  const navigate = targetId => setActiveSection(targetId);
  useEffect(() => {
    ...
    ]).then(async ([projectResult, paymentResult, configResult, receiptResult, supervisorResult]) => {
      setProject(projectResult.data || null); setPayment(paymentResult.data || null); setReceipt(receiptResult.data || null); setSupervisor(supervisorResult?.data || null); setClearanceFee(configResult?.clearance_fee_kobo || 200000); setMaxPdfBytes(Number(configResult?.max_pdf_size_bytes) || DEFAULT_MAX_PDF_BYTES);
      if (projectResult.data) {
        const [versionsResult, reviewsResult] = await Promise.all([
          supabase.from('project_versions').select('id,version_number,file_name,file_path,title,abstract,degree,source,change_summary,created_at').eq('project_id', projectResult.data.id).order('version_number', { ascending: true }),
          supabase.from('project_reviews').select('id,action,comment,annotations,version_number,created_at').eq('project_id', projectResult.data.id).order('created_at', { ascending: false }),
        ]);
        if (versionsResult.error) throw versionsResult.error;
        if (reviewsResult.error) throw reviewsResult.error;
        setProjectVersions(versionsResult.data || []); setProjectReviews(reviewsResult.data || []);
        setForm({ title: projectResult.data.title || '', degree: projectResult.data.degree || 'BSc', abstract: projectResult.data.abstract || '' });
      }
    }).catch(error => onToast(error.message || 'Student workspace could not be loaded.')).finally(() => setLoading(false));
    return undefined;
  ...
  const submit = async event => {
    ...
    if (project && project.status !== 'revision_requested') { onToast('Your project is already in the review workflow. Wait for supervisor feedback before uploading another version.'); return; }
    ...
    try {
      if (project?.status === 'revision_requested' && payment) {
        ...
        setProject(data.project); setProjectVersions(versions => [...versions, { version_number: data.version_number, source: 'resubmission', file_name: selectedFile.name, title: form.title, created_at: new Date().toISOString(), change_summary: 'Corrected thesis resubmitted after supervisor feedback.' }]); setProjectReviews(reviews => [{ id: `local-${Date.now()}`, action: 'submitted', comment: 'Revision resubmitted for another supervisor review.', version_number: data.version_number, created_at: new Date().toISOString() }, ...reviews]); setFile(null); setSubmitting(false); onToast('Revision resubmitted. Supervisor notified. No second payment was taken.'); return;
      }
  ...
  return <StudentWorkspacePages profile={profile} session={session} preview={preview} onToast={onToast} onProfileUpdate={onProfileUpdate} activeSection={activeSection} navigate={navigate} project={project} payment={payment} receipt={receipt} supervisor={supervisor} projectVersions={projectVersions} projectReviews={projectReviews} clearanceFee={clearanceFee} maxPdfBytes={maxPdfBytes} submitting={submitting} pendingVerification={pendingVerification} receiptQrUrl={receiptQrUrl} form={form} setForm={setForm} file={file} setFile={setFile} submit={submit} retryPendingVerification={retryPendingVerification} generateReceipt={generateReceipt} />;
  return <Workspace
    role="student"
...
function StudentWorkspacePages({ profile, session, preview, onToast, onProfileUpdate, activeSection, navigate, project, payment, receipt, supervisor, projectVersions, projectReviews, clearanceFee, maxPdfBytes, submitting, pendingVerification, receiptQrUrl, form, setForm, file, setFile, submit, retryPendingVerification, generateReceipt }) {
  const [paymentEvidenceOpen, setPaymentEvidenceOpen] = useState(false);
  const sidebar = [
    [GraduationCap, 'Overview', activeSection === 'student-overview', () => navigate('student-overview')],
    [FileText, 'Submission', activeSection === 'student-submission', () => navigate('student-submission')],
    [CircleDollarSign, 'Payments', activeSection === 'student-payments', () => navigate('student-payments')],
    [ShieldCheck, 'Receipt', activeSection === 'student-receipt', () => navigate('student-receipt')],
  ];
  const workflowLabel = project ? project.status.replaceAll('_', ' ') : pendingVerification ? 'Payment verification pending' : 'Not started';
  const submissionLabel = project ? 'Received' : pendingVerification ? 'Verification pending' : 'Awaiting upload';
  const receiptLabel = project?.status === 'cleared' || receipt ? 'Ready' : 'After clearance';
  return <Workspace role="student" title="Your clearance workspace" subtitle="A focused path for submission, review progress, payment evidence, and digital clearance." sidebar={sidebar} profile={profile} session={session} preview={preview} onProfileUpdate={onProfileUpdate} onToast={onToast}>
    {project && activeSection !== 'student-overview' && <StudentReviewStatusCard project={project} reviews={projectReviews} />}
    {activeSection === 'student-overview' && <div className="workspace-section" id="student-overview">
      <StudentProfileCard profile={profile} />
      <MetricCards items={[["Workflow status", workflowLabel, FileCheck2], ["Clearance fee", formatNaira(clearanceFee), CircleDollarSign], ["Submission", submissionLabel, FileText], ["Receipt", receiptLabel, Archive]]} />
      {project && <StudentReviewStatusCard project={project} reviews={projectReviews} />}
      <div className="workspace-grid student-overview-grid"><SupervisorContactCard supervisor={supervisor} /><section className="surface student-focus-panel"><div className="surface-head"><div><p className="eyebrow">Current progress</p><h2>{project ? project.title : 'Start your clearance journey'}</h2><p>{project ? 'Your project is moving through the connected review process.' : 'Submit your project once, then follow every stage from this workspace.'}</p></div><LockKeyhole size={18} color="#065f46" /></div>{project ? <div className="status-panel"><StatusChip status={project.status} /><div className="progress-track"><div className="progress-fill" style={{ width: `${['submitted', 'supervisor_review', 'revision_requested'].includes(project.status) ? 34 : project.status === 'supervisor_approved' ? 62 : project.status === 'library_review' ? 80 : 100}%` }} /></div>{project.revision_note && <p className="helper"><strong>Supervisor note:</strong> {project.revision_note}</p>}</div> : <EmptyState icon={FileText} title="No submission yet" copy="Open Submission to add your research details and PDF." />}<button className="button button-ghost button-small student-inline-action" type="button" onClick={() => project?.status === 'revision_requested' ? navigate('student-submission') : project ? setPaymentEvidenceOpen(true) : navigate('student-submission')}><ArrowRight size={14} />{project?.status === 'revision_requested' ? 'Open revision form' : project ? 'View payment evidence' : 'Start submission'}</button></section></div>
      {project && <ProjectFeedbackPanel versions={projectVersions} reviews={projectReviews} />}    </div>}
    {activeSection === 'student-submission' && <section className="surface workspace-section" id="student-submission"><div className="surface-head"><div><p className="eyebrow">Research submission</p><h2>Submission details</h2><p>Capture the metadata your supervisor and library will verify.</p></div>{project && <StatusChip status={project.status} />}</div>{project?.status === 'revision_requested' && <div className="revision-banner"><RefreshCw size={16} /><div><strong>Revision Required</strong><span>Upload the corrected PDF and resubmit without paying the clearance fee again. Your previous version remains part of the official record.</span></div></div>}{pendingVerification && <div className="pending-payment-banner"><div><strong>Payment verification pending</strong><span>Your PDF is safely uploaded. Retry verification with the same Paystack reference; you will not be charged again.</span></div><button className="button button-primary button-small" type="button" onClick={retryPendingVerification} disabled={submitting}><RefreshCw size={14} />{submitting ? 'Retrying...' : 'Retry verification'}</button></div>}<form className="form-grid" onSubmit={submit}><div className="field full"><label htmlFor="project-title-input">Project title</label><input id="project-title-input" disabled={Boolean(project && project.status !== 'revision_requested')} value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></div><div className="field"><label htmlFor="project-degree-input">Degree</label><select id="project-degree-input" disabled={Boolean(project && project.status !== 'revision_requested')} value={form.degree} onChange={e => setForm({ ...form, degree: e.target.value })}><option>BSc</option><option>PGD</option><option>MSc</option><option>PhD</option></select></div><div className="field"><label htmlFor="thesis-pdf-input">Thesis PDF</label><input id="thesis-pdf-input" disabled={Boolean(project && project.status !== 'revision_requested')} type="file" accept="application/pdf,.pdf" onChange={e => { const selected = e.target.files?.[0] || null; setFile(selected); const error = validateThesisFile(selected, maxPdfBytes); if (error) onToast(error); }} /><span className="helper">PDF only. Maximum {Math.round(maxPdfBytes / (1024 * 1024))} MB.</span></div><div className="field full"><label htmlFor="project-abstract-input">Abstract</label><textarea id="project-abstract-input" disabled={Boolean(project && project.status !== 'revision_requested')} value={form.abstract} onChange={e => setForm({ ...form, abstract: e.target.value })} /><span className="helper">Minimum 50 characters. This abstract becomes part of the public catalog after library approval.</span></div><div className="modal-actions field full"><button className="button button-primary" id="pay-btn" disabled={submitting || Boolean(pendingVerification) || Boolean(project && project.status !== 'revision_requested')}>{project?.status === 'revision_requested' && payment ? <><RefreshCw size={15} />{submitting ? 'Submitting revision...' : 'Upload Revision & Resubmit'}</> : project ? <><Check size={15} />Awaiting supervisor review</> : pendingVerification ? <><RefreshCw size={15} />Payment verification pending</> : <><CircleDollarSign size={15} />{submitting ? 'Opening secure checkout...' : 'Pay & submit thesis'}</>}</button></div></form></section>}
    {activeSection === 'student-payments' && <StudentPaymentEvidence payment={payment} project={project} receipt={receipt} receiptQrUrl={receiptQrUrl} generateReceipt={generateReceipt} onReceipt={() => navigate('student-receipt')} pendingVerification={pendingVerification} retryPendingVerification={retryPendingVerification} submitting={submitting} />}
    {activeSection === 'student-receipt' && <StudentReceiptPage payment={payment} project={project} receipt={receipt} receiptQrUrl={receiptQrUrl} generateReceipt={generateReceipt} profile={profile} />}
    <Modal open={paymentEvidenceOpen} onClose={() => setPaymentEvidenceOpen(false)} eyebrow="Payment evidence" title="Payment captured">
      <StudentPaymentEvidence payment={payment} project={project} receipt={receipt} receiptQrUrl={receiptQrUrl} generateReceipt={generateReceipt} onReceipt={() => { setPaymentEvidenceOpen(false); navigate('student-receipt'); }} pendingVerification={pendingVerification} retryPendingVerification={retryPendingVerification} submitting={submitting} />
      <div className="modal-actions"><button className="button button-ghost" type="button" onClick={() => setPaymentEvidenceOpen(false)}>Close</button><button className="button button-primary" type="button" onClick={() => { setPaymentEvidenceOpen(false); navigate('student-payments'); }}><CircleDollarSign size={15} />Open full payment history</button></div>
    </Modal>
  </Workspace>;
}

function reviewActionLabel(action) {
  return { submitted: 'Submitted for review', approved: 'Accepted by supervisor', revision_requested: 'Corrections requested', rejected: 'Rejected by supervisor' }[action] || String(action || 'Review update').replaceAll('_', ' ');
}

function latestSupervisorReview(reviews = []) {
  return reviews.find(item => ['approved', 'revision_requested', 'rejected'].includes(item.action));
}

function StudentReviewStatusCard({ project, reviews = [] }) {
  const latestEvent = reviews[0];
  const review = ['approved', 'revision_requested', 'rejected'].includes(latestEvent?.action) ? latestEvent : null;
  const approved = project.status === 'supervisor_approved' || review?.action === 'approved';
  const revision = project.status === 'revision_requested' || review?.action === 'revision_requested' || project.status === 'rejected' || review?.action === 'rejected';
  const tone = approved ? 'is-approved' : revision ? 'is-revision' : 'is-pending';
  const heading = approved ? 'Supervisor accepted your project' : revision ? 'Supervisor feedback is ready' : 'Your project is with the supervisor';
  const copy = approved ? 'Your approved document has moved to the library verification stage.' : revision ? 'Read the reason and the marked document, correct your work, then resubmit a new version.' : 'You will see the decision, comments, and marked document here when the review is complete.';
  return <section className={`student-review-status ${tone}`}>
    <div className="student-review-status-icon">{approved ? <CheckCircle2 size={20} /> : revision ? <PencilLine size={20} /> : <RefreshCw size={20} />}</div>
    <div className="student-review-status-copy"><p className="eyebrow">Supervisor review</p><h2>{heading}</h2><p>{copy}</p>{review?.comment && <div className="student-review-status-note"><MessageCircle size={15} /><span>{review.comment}</span></div>}<small>{review ? `${reviewActionLabel(review.action)} · ${displayDate(review.created_at)}` : 'Awaiting supervisor decision'}</small></div>
    <StatusChip status={project.status} />
  </section>;
}

function ProjectFeedbackPanel({ versions = [], reviews = [] }) {
  const [selectedReview, setSelectedReview] = useState(null);
  const latest = versions[versions.length - 1];
  const latestDecision = latestSupervisorReview(reviews);
  const orderedReviews = [...reviews].sort((a, b) => new Date(a.created_at || 0) - new Date(b.created_at || 0));
  const reviewedVersion = latestDecision && versions.find(version => Number(version.version_number) === Number(latestDecision.version_number));
  return <section className="surface project-feedback-panel">
    <div className="surface-head"><div><p className="eyebrow">Review feedback</p><h2>Review activity and versions</h2><p>Every upload is immutable. Follow what happened, open the exact PDF your supervisor reviewed, and resubmit only when corrections are requested.</p></div><span className="tag"><RefreshCw size={12} />{versions.length ? 'Latest v' + versions[versions.length - 1].version_number : 'Version tracking'}</span></div>
    {latestDecision?.comment && <div className={`revision-banner ${latestDecision.action === 'approved' ? 'is-approved' : ''}`}><PencilLine size={16} /><div><strong>{reviewActionLabel(latestDecision.action)}</strong><span>{latestDecision.comment}</span></div></div>}
    {latestDecision?.annotations?.length > 0 && <div className="student-latest-review"><div className="student-latest-review-head"><div><strong>What your supervisor marked</strong><span>{latestDecision.annotations.length} mark{latestDecision.annotations.length === 1 ? '' : 's'} on version {latestDecision.version_number || '—'}</span></div>{reviewedVersion && <button className="button button-primary button-small" type="button" onClick={() => setSelectedReview({ review: latestDecision, version: reviewedVersion })}><Eye size={14} />Open reviewed document</button>}</div><ReviewAnnotationPreview annotations={latestDecision.annotations} /></div>}
    <div className="review-timeline-heading"><div><p className="eyebrow">Latest work</p><h3>Supervisor activity</h3></div><span className="tag">{orderedReviews.length} event{orderedReviews.length === 1 ? '' : 's'}</span></div>
    {orderedReviews.length ? <div className="review-timeline">{orderedReviews.map((review, index) => {
      const version = versions.find(item => Number(item.version_number) === Number(review.version_number));
      const annotations = review.annotations || [];
      return <article className={`review-timeline-item ${review.action === 'revision_requested' || review.action === 'rejected' ? 'is-attention' : review.action === 'approved' ? 'is-success' : ''}`} key={review.id || `${review.action}-${index}`}><span className="review-timeline-marker">{review.action === 'approved' ? <Check size={14} /> : review.action === 'revision_requested' || review.action === 'rejected' ? <PencilLine size={14} /> : <FileText size={14} />}</span><div className="review-timeline-content"><div className="review-timeline-top"><div><strong>{reviewActionLabel(review.action)}</strong><span>{review.version_number ? `Version ${review.version_number}` : 'Workflow event'} · {displayDate(review.created_at)}</span></div>{version && <button className="button button-ghost button-small" type="button" onClick={() => setSelectedReview({ review, version })}><Eye size={14} />View document</button>}</div>{review.comment && <p>{review.comment}</p>}{annotations.length > 0 && <span className="tag"><PencilLine size={12} />{annotations.length} document mark{annotations.length === 1 ? '' : 's'}</span>}</div></article>;
    })}</div> : <EmptyState icon={RefreshCw} title="No supervisor activity yet" copy="The review timeline will appear after your submission is received." />}
    <div className="review-version-heading"><strong>Immutable version history</strong><span className="tag">{versions.length || 0} saved</span></div>
    {versions.length ? <div className="version-history">{versions.map(version => <div className="version-history-item" key={version.id || version.version_number}><div><strong>Version {version.version_number}</strong><span>{version.source === 'resubmission' ? 'Corrected resubmission for another review' : 'Initial submission'} · {version.file_name || 'PDF upload'}</span></div><div><span className="tag">{version.source === 'resubmission' ? 'RESUBMITTED' : 'ORIGINAL'}</span><small>{displayDate(version.created_at)}</small></div></div>)}</div> : <EmptyState icon={FileText} title="Version history will appear here" copy="Your first successful submission becomes version 1." />}
    {latest && <details className="student-submission-preview"><summary><Eye size={14} />View latest submission</summary><PdfPreview path={latest.file_path} /></details>}
    <StudentReviewedDocumentModal selected={selectedReview} onClose={() => setSelectedReview(null)} />
  </section>;
}

function annotationLabel(mark) {
  const type = mark.annotation_type || mark.type;
  if (type === 'highlight' || type === 'text') return mark.content?.text ? `Highlighted: "${mark.content.text}"` : 'Highlighted text';
  if (type === 'circle') return 'Circled correction area';
  if (type === 'arrow') return 'Arrow pointing to a correction';
...
function StudentReviewedDocumentModal({ selected, onClose }) {
  ...
  return <Modal open={Boolean(selected)} onClose={onClose} wide eyebrow={review ? reviewActionLabel(review.action) : 'Supervisor review'} title={version ? `Version ${version.version_number} · ${version.file_name || 'Reviewed project document'}` : 'Reviewed project document'} className="student-reviewed-document-modal">
    <div className="student-reviewed-document-head"><div><p className="helper">This is the immutable document version attached to the review event. Your supervisor's marks are read-only.</p>{review?.comment && <div className="student-review-status-note"><MessageCircle size={15} /><span>{review.comment}</span></div>}</div><span className="tag"><LockKeyhole size={12} />Read only</span></div>
    <ReadOnlyDocumentReview path={version?.file_path} annotations={review?.annotations || []} />
    <div className="modal-actions"><button className="button button-primary" type="button" onClick={onClose}>Close document</button></div>
  </Modal>;
}
...
function TeacherWorkspace({ profile, session, preview, onToast, onProfileUpdate }) {
  const [projects, setProjects] = useState(preview ? demoReviewProjects.map((item, index) => ({ ...item, versions: [{ id: 'preview-version-1-' + index, version_number: 1, source: 'initial', file_name: item.title.toLowerCase().replaceAll(' ', '-') + '.pdf', title: item.title, abstract: item.abstract, degree: item.degree, created_at: new Date().toISOString() }, ...(item.status === 'revision_requested' ? [{ id: 'preview-version-2-' + index, version_number: 2, source: 'resubmission', file_name: item.title.toLowerCase().replaceAll(' ', '-') + '-corrected.pdf', title: item.title, abstract: item.abstract, degree: item.degree, created_at: new Date().toISOString(), change_summary: 'Corrected version submitted for another review.' }] : [])] })) : []);
  ...
  const navigate = targetId => setActiveSection(targetId);
  // A revision request is waiting on the student, so it returns to the queue
  // only after the student resubmits and the workflow becomes supervisor_review.
  const canReview = status => ['submitted', 'supervisor_review'].includes(status);
  useEffect(() => {
    if (preview || !session || !supabase) return undefined;
    ...
    Promise.all([
    ...
    ]).then(async ([projectsResult, studentsResult]) => {
      if (projectsResult.error) throw projectsResult.error; if (studentsResult.error) throw studentsResult.error;
      ...
      if (versionsResult.error) throw versionsResult.error;
      const reviewsResult = loadedProjects.length ? await supabase.from('project_reviews').select('id,project_id,action,comment,annotations,version_number,created_at').in('project_id', loadedProjects.map(item => item.id)).order('created_at', { ascending: false }) : { data: [], error: null };
      if (reviewsResult.error) throw reviewsResult.error;
      ...
      setProjects(loadedProjects.map(item => ({ ...item, author: item.profiles?.full_name, matric: item.profiles?.matric, dept: item.profiles?.department, versions: versionMap[item.id] || [], review_annotations: latestReviewMap[item.id]?.annotations || [], review_comment: latestReviewMap[item.id]?.comment || item.revision_note || '', isDemo: false }))); setStudents(studentsResult.data || []);
  ...
  const openReview = item => { setSelected(item); setComment(canReview(item.status) ? '' : item.review_comment || item.revision_note || ''); setAnnotations(item.review_annotations || []); };
  if (loading) return <PageSkeleton role="teacher" />;
  return <SupervisorWorkspacePage profile={profile} session={session} preview={preview} onProfileUpdate={onProfileUpdate} onToast={onToast} activeSection={activeSection} navigate={navigate} projects={projects} students={students} reviewProjects={reviewProjects} historyProjects={historyProjects} canReview={canReview} openReview={openReview} selected={selected} setSelected={setSelected} comment={comment} setComment={setComment} annotations={annotations} setAnnotations={setAnnotations} review={review} />;
...
function SupervisorWorkspacePage({ profile, session, preview, onProfileUpdate, onToast, activeSection, navigate, projects, students, reviewProjects, historyProjects, canReview, openReview, selected, setSelected, comment, setComment, annotations, setAnnotations, review }) {
  const sidebar = [
    [Archive, 'Overview', activeSection === 'teacher-overview', () => navigate('teacher-overview')],
    [FileCheck2, 'Assigned projects', activeSection === 'teacher-projects', () => navigate('teacher-projects')],
    [Users, 'Assigned students', activeSection === 'teacher-students', () => navigate('teacher-students')],
    [CheckCircle2, 'Review history', activeSection === 'teacher-history', () => navigate('teacher-history')],
  ];

  return <Workspace role="teacher" title="Supervisor review queue" subtitle="A focused workspace for assigned research, student support, and auditable decisions." sidebar={sidebar} profile={profile} session={session} preview={preview} onProfileUpdate={onProfileUpdate} onToast={onToast}>
    {activeSection === 'teacher-overview' && <div className="workspace-section" id="teacher-overview">
      <MetricCards items={[["Awaiting review", reviewProjects.filter(item => item.status === 'supervisor_review').length, FileCheck2], ["Revision requests", projects.filter(item => item.status === 'revision_requested').length, RefreshCw], ["Approved", projects.filter(item => item.status === 'supervisor_approved').length, CheckCircle2], ["Assigned students", students.length, Users]]} />
      <div className="workspace-grid supervisor-overview-grid">
        <section className="surface supervisor-focus-panel"><div className="surface-head"><div><p className="eyebrow">Today's focus</p><h2>Projects awaiting your review</h2><p>Open a project to inspect its private PDF and record a decision.</p></div><span className="tag"><LockKeyhole size={12} />RLS protected</span></div>{reviewProjects.length ? <div className="supervisor-focus-list">{reviewProjects.slice(0, 3).map(item => <div className="supervisor-focus-item" key={item.id}><div><strong>{item.title}</strong><span>{item.author || 'Student'} - {item.matric || 'Matric pending'}</span></div><div className="card-actions"><StatusChip status={item.status} /><button className="button button-primary button-small" type="button" onClick={() => openReview(item)}>Review</button></div></div>)}</div> : <EmptyState icon={FileCheck2} title="Review queue is clear" copy="New assigned submissions will appear here." />}<button className="button button-ghost button-small supervisor-inline-action" type="button" onClick={() => navigate('teacher-projects')}><ArrowRight size={14} />Open all assigned projects</button></section>
        <section className="surface supervisor-focus-panel"><div className="surface-head"><div><p className="eyebrow">Academic support</p><h2>Supervisor profile</h2><p>Your official contact details are available to assigned students.</p></div><UserCheck size={18} color="#065f46" /></div><div className="supervisor-profile-compact"><ProfileAvatar name={profile?.full_name} src={profile?.avatar_url} size="avatar-medium" /><div><h3>{profile?.full_name || 'Supervisor'}</h3><ContactDetails person={profile} roleLabel="Supervisor account" /></div></div><p className="helper supervisor-profile-hint">Use the profile button at the bottom of the sidebar to view or edit your contact details.</p></section>
      </div>
    </div>}
    {activeSection === 'teacher-projects' && <section className="surface workspace-section" id="teacher-projects"><SectionHeader eyebrow="Review workspace" title="Assigned projects" copy="Review assigned research, inspect private PDFs, and submit an approval or revision request." action={<span className="tag">{reviewProjects.length} awaiting action</span>} /><SupervisorProjectTable projects={projects} canReview={canReview} onReview={openReview} /></section>}
    {activeSection === 'teacher-students' && <section className="surface workspace-section" id="teacher-students"><SectionHeader eyebrow="Student support" title="Assigned students" copy="Keep each student's academic identity and contact details close for follow-up." action={<span className="tag"><LockKeyhole size={12} />RLS protected</span>} />{students.length ? <div className="contact-directory">{students.map(student => <div className="contact-directory-item" key={student.id}><div className="identity-cell"><ProfileAvatar name={student.full_name} src={student.avatar_url} size="avatar-medium" /><div><strong>{student.full_name || 'Student'}</strong><span className="helper">{student.matric || 'Matric pending'} - {student.department || 'Department pending'}</span></div></div><ContactDetails person={student} roleLabel="Assigned student" /></div>)}</div> : <EmptyState icon={Users} title="No assigned students" copy="Students assigned by an administrator will appear here." />}</section>}
    {activeSection === 'teacher-history' && <section className="surface workspace-section" id="teacher-history"><SectionHeader eyebrow="Audit trail" title="Review history" copy="Approved projects and revision requests remain visible for follow-up." />{historyProjects.length ? <div className="queue-list">{historyProjects.map(item => <div className="queue-item" key={`history-${item.id}`}><div className="identity-cell"><ProfileAvatar name={item.author} src={item.profiles?.avatar_url} /><div><h3>{item.title}</h3><p>{item.author || 'Student'} - {item.matric || 'Matric pending'}</p></div></div><div className="card-actions"><StatusChip status={item.status} /><button className="button button-ghost button-small" type="button" onClick={() => openReview(item)}><Eye size={14} />View project</button></div></div>)}</div> : <EmptyState icon={FileCheck2} title="No decisions yet" copy="Approved projects and revision requests will appear here." />}</section>}
    <SupervisorReviewModal selected={selected} setSelected={setSelected} preview={preview} comment={comment} setComment={setComment} annotations={annotations} setAnnotations={setAnnotations} canReview={canReview} review={review} />
  </Workspace>;
}
...
function SupervisorReviewModal({ selected, setSelected, preview, comment, setComment, annotations, setAnnotations, canReview, review }) {
  ...
  const readOnly = Boolean(selected && !canReview(selected.status));
  const version = selected?.versions?.[selected.versions.length - 1]?.version_number || 1;
  return <Modal open={Boolean(selected)} onClose={() => setSelected(null)} eyebrow={readOnly ? 'Read-only project view' : (fullView ? 'Document review' : 'Supervisor review')} title={selected?.title || ''} wide className={fullView ? 'document-review-modal' : ''}>
    {!fullView ? <div className="review-modal-body">
      <div className="review-modal-toolbar"><div><p className="eyebrow">Version {version} {readOnly ? 'record' : 'review'}</p><p className="helper">{readOnly ? 'This project decision is complete. The document and saved supervisor marks are read-only.' : 'Add comments and draw arrows to point to corrections. The student\'s original upload remains immutable.'}</p></div>{readOnly ? <span className="tag"><LockKeyhole size={12} />Read only</span> : <button className="button button-ghost button-small" type="button" onClick={() => setFullView(true)}><Eye size={14} />Open full view</button>}</div>
      <div className="modal-body-grid"><div><div className="status-panel"><div className="identity-cell"><ProfileAvatar name={selected?.author} src={selected?.profiles?.avatar_url} size="avatar-medium" /><div><h3>{selected?.author || 'Student'}</h3><p className="helper">{selected?.matric} - {selected?.dept || 'Computer Science'} - {selected?.degree}</p><ContactDetails person={selected?.profiles} roleLabel="Student contact" /></div></div><StatusChip status={selected?.status} /></div><p className="helper review-abstract">{selected?.abstract}</p><ReviewVersionPanel versions={selected?.versions || []} />{readOnly ? (comment && <div className="review-readonly-comment"><strong>Supervisor decision comment</strong><p>{comment}</p></div>) : <div className="field review-comment-field"><label htmlFor="modal-review-comment">Correction or decision comment</label><textarea id="modal-review-comment" value={comment} onChange={e => setComment(e.target.value)} placeholder="Explain the correction needed or record your approval note." /></div>}</div><div><p className="eyebrow">{readOnly ? 'Read-only PDF view' : 'Private PDF review surface'}</p><PdfReviewViewer path={selected?.isDemo ? '' : selected?.file_path} annotations={annotations} onChange={setAnnotations} readOnly={readOnly} /></div></div>
      {!readOnly && <div className="modal-actions"><button className="button button-ghost" onClick={() => review('request_revision')}><PencilLine size={15} />Request revision & send corrections</button><button className="button button-primary" onClick={() => review('approve')}><Check size={15} />Approve Project</button></div>}
    </div> : <div className="document-review-workspace">
      <div className="document-review-topbar"><div><p className="eyebrow">Version {version} · {readOnly ? 'read-only document' : 'private document'}</p><span className="helper">{readOnly ? 'This completed review is locked. Saved highlights and comments can be viewed but not changed.' : 'Review the PDF at full size. Select text to highlight it, or drag on the page to circle or point to a correction.'}</span></div><button className="button button-ghost button-small" type="button" onClick={() => setFullView(false)}><Eye size={14} />Exit full view</button></div>
      <PdfReviewViewer path={selected?.isDemo ? '' : selected?.file_path} annotations={annotations} onChange={setAnnotations} fullView readOnly={readOnly} />
      {readOnly ? <div className="document-review-footer"><span className="tag"><LockKeyhole size={12} />Read-only review record</span></div> : <div className="document-review-footer"><div className="field document-review-comment"><label htmlFor="fullscreen-review-comment">Correction or decision comment</label><textarea id="fullscreen-review-comment" value={comment} onChange={e => setComment(e.target.value)} placeholder="Explain the correction needed or record your approval note." /></div><div className="document-review-actions"><button className="button button-ghost" onClick={() => review('request_revision')}><PencilLine size={15} />Request revision & send corrections</button><button className="button button-primary" onClick={() => review('approve')}><Check size={15} />Approve Project</button></div></div>}
    </div>}
  </Modal>;
}

function ReviewVersionPanel({ versions = [] }) {
  const latest = versions[versions.length - 1];
  const previous = versions[versions.length - 2];
  const differences = previous && latest ? [
    latest.file_path !== previous.file_path && 'PDF file uploaded',
    latest.title !== previous.title && 'Project title updated',
    latest.abstract !== previous.abstract && 'Abstract updated',
    latest.degree !== previous.degree && 'Degree metadata updated',
  ].filter(Boolean) : [];
  return <div className="review-version-panel"><div className="review-version-heading"><strong>Version history</strong><span className="tag">{versions.length || 1} saved</span></div>{versions.length ? <div className="review-version-list">{versions.map(version => <div className="review-version-item" key={version.id || version.version_number}><div><strong>v{version.version_number} {version.source === 'resubmission' ? 'Resubmission' : 'Original'}</strong><span>{version.file_name || 'PDF upload'} - {displayDate(version.created_at)}</span></div>{version.version_number === latest?.version_number && <span className="tag">CURRENT</span>}</div>)}</div> : <p className="helper">Version history will appear after the versioning migration is run.</p>}{previous && <div className="review-diff"><small>Changes since v{previous.version_number}</small>{differences.length ? differences.map(change => <span key={change}><Check size={12} />{change}</span>) : <span>No metadata changes detected. Review the new PDF.</span>}</div>}</div>;
}

function PdfReviewViewer({ path, annotations = [], onChange, fullView, readOnly = false }) {
  if (readOnly) return <ReadOnlyDocumentReview path={path} annotations={annotations} />;
...
function LegacyPdfReviewViewer({ path, annotations = [], onChange, fullView }) {
  ...
  useEffect(() => { let active = true; if (!path) { setUrl(''); return undefined; } signedPdfUrl(path).then(value => { if (active) setUrl(value); }).catch(err => { if (active) setError(err.message); }); return () => { active = false; }; }, [path]);
  const point = event => { const rect = stageRef.current?.getBoundingClientRect(); if (!rect) return null; return { x: ((event.clientX - rect.left) / rect.width) * 100, y: ((event.clientY - rect.top) / rect.height) * 100 }; };
  const begin = event => { if (!drawing) return; event.currentTarget.setPointerCapture?.(event.pointerId); setStart(point(event)); };
  const move = event => { if (drawing && start) setDraft({ ...start, ...point(event) }); };
...
function SupervisorProjectTable({ projects, canReview, onReview }) {
  if (!projects.length) return <EmptyState icon={FileText} title="No assigned projects" copy="New submissions will appear here when they are assigned to you." />;
  return <div className="table-wrap"><table className="data-table"><thead><tr><th>Student</th><th>Contact</th><th>Project</th><th>Status</th><th>Action</th></tr></thead><tbody>{projects.map(item => <tr key={item.id}><td><div className="identity-cell"><ProfileAvatar name={item.author} src={item.profiles?.avatar_url} /><div><strong>{item.author || 'Student'}</strong><br /><span className="helper">{item.matric || 'Matric pending'}</span></div></div></td><td><span className="helper">{item.profiles?.email || 'Email not provided'}<br />{item.profiles?.phone || 'Phone not provided'}</span></td><td><strong>{item.title}</strong><br /><span className="helper">{item.dept || item.department || 'Department pending'} - {item.degree || 'Degree pending'}</span></td><td><StatusChip status={item.status} /></td><td>{canReview(item.status) ? <button className="button button-primary button-small" type="button" onClick={() => onReview(item)}>Review</button> : <button className="button button-ghost button-small" type="button" onClick={() => onReview(item)}><Eye size={14} />View</button>}</td></tr>)}</tbody></table></div>;
}
...
function LibraryWorkspace({ profile, session, preview, onToast }) {
  ...
  const [departments, setDepartments] = useState(preview ? [{ id: 'preview-department', name: 'Computer Science', code: 'CSC' }] : []);
  const [courses, setCourses] = useState(preview ? [{ id: 'preview-course', name: 'Computer Science', code: 'CSC-BSC', department_id: 'preview-department' }] : []);
  const [loading, setLoading] = useState(!preview);
  const [selected, setSelected] = useState(null);
  const [form, setForm] = useState({ title: '', abstract: '', degree: '', department_id: '', course_id: '', keywords: '', shelf_number: '', doi: '', comment: '', library_note: '' });
  const [qrUrl, setQrUrl] = useState('');
  const [queueQuery, setQueueQuery] = useState('');
  const [queueStatus, setQueueStatus] = useState('all');
  const [queueDepartment, setQueueDepartment] = useState('all');
  const [queueCourse, setQueueCourse] = useState('all');
  const [queueDegree, setQueueDegree] = useState('all');
  const [queueDate, setQueueDate] = useState('all');
  ...
  useEffect(() => {
    if (!preview || previewAction !== 'open_catalog_record' || !projects.length || selected) return;
    const item = projects[0];
    setSelected(item);
    setForm({ title: item.title || '', abstract: item.abstract || '', degree: item.degree || 'BSc', department_id: item.department_id || 'preview-department', course_id: item.course_id || 'preview-course', keywords: '', shelf_number: item.shelf_number || 'KASU-CS-001', doi: item.doi || '', comment: 'Preview catalog note for automated library interaction coverage.', library_note: 'Preview catalog note for automated library interaction coverage.' });
  }, [preview, projects, selected]);
  ...
  useEffect(() => {
    ...
    const courseQuery = supabase.from('courses').select('id,name,code,level,department_id').eq('institution_id', profile.institution_id).order('name');
    Promise.all([projectQuery, reviewQuery, receiptQuery, departmentQuery, courseQuery]).then(([projectResult, reviewResult, receiptResult, departmentResult, courseResult]) => {
      const failed = [projectResult, reviewResult, receiptResult, departmentResult, courseResult].find(result => result.error);
      if (failed?.error) throw failed.error;
      const reviews = reviewResult.data || [];
      const receipts = receiptResult.data || [];
      const reviewMap = reviews.reduce((map, review) => { (map[review.project_id] ||= []).push(review); return map; }, {});
      const receiptMap = receipts.reduce((map, receipt) => { map[receipt.project_id] = receipt; return map; }, {});
      if (!active) return;
      setDepartments(departmentResult.data || []);
      setCourses(courseResult.data || []);
      setProjects((projectResult.data || []).map(item => ({ ...item, author: item.profiles?.full_name, matric: item.profiles?.matric, dept: item.departments?.name || item.profiles?.department, course_name: item.courses?.name, reviewHistory: reviewMap[item.id] || [], receipt: receiptMap[item.id] || null })));
    }).catch(error => { if (active) { setProjects([]); onToast(error.message || 'Library records could not be loaded.'); } }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [onToast, preview, profile, session]);
  ...
  const openRecord = item => { setSelected(item); setForm({ title: item.title || '', abstract: item.abstract || '', degree: item.degree || '', department_id: item.department_id || item.departments?.id || '', course_id: item.course_id || item.courses?.id || '', keywords: Array.isArray(item.keywords) ? item.keywords.join(', ') : '', shelf_number: item.shelf_number || `KASU-CS-${String(projects.indexOf(item) + 1).padStart(3, '0')}`, doi: item.doi || '', comment: '', library_note: item.library_note || '' }); };
  const saveMetadata = async silent => {
    if (!selected) return null;
    if (!libraryProjectMetadataComplete(selected, form)) throw new Error('Title, abstract, degree, department, and a PDF are required.');
    if (preview || selected.isDemo) { const updated = { ...selected, ...form, keywords: libraryKeywords(form.keywords), dept: departments.find(item => item.id === form.department_id)?.name || selected.dept, status: selected.status }; updateProject(updated); if (!silent) onToast('Metadata saved in preview.'); return updated; }
    setBusyAction('save');
    try { const data = await invoke('project-workflow', { action: 'library_update_metadata', project_id: selected.id, title: form.title, abstract: form.abstract, degree: form.degree, department_id: form.department_id, course_id: form.course_id || null, keywords: libraryKeywords(form.keywords), library_note: form.library_note }); updateProject(data.project); if (!silent) onToast('Library metadata saved.'); return data.project; } finally { setBusyAction(''); }
  };
  const verifyMetadata = async () => {
    if (!selected) return;
    try {
      await saveMetadata(true);
      if (preview || selected.isDemo) { const updated = { ...selected, status: 'library_review', metadata_verified_at: new Date().toISOString(), library_verified_at: new Date().toISOString(), library_verified_by: 'preview-library' }; updateProject(updated); onToast('Metadata verified. The record is ready for publication.'); return; }
  ...
  const publish = async () => {
    ...
    try {
      ...
      setBusyAction('publish'); const data = await invoke('project-workflow', { action: 'library_publish', project_id: selected.id, shelf_number: form.shelf_number.trim(), doi: form.doi, comment: form.comment || form.library_note }); updateProject(data.project); onToast('Project published to the public catalogue and QR code generated.');
    } catch (error) { onToast(error.message || 'Project could not be published.'); } finally { setBusyAction(''); }
  };
  const issueClearanceReceipt = async () => {
    if (!selected || !['published', 'cleared'].includes(selected.status)) return;
    try {
      if (preview || selected.isDemo) { const receipt = { id: `preview-receipt-${selected.id}`, project_id: selected.id, verification_code: 'SPMS-PREVIEW-LIBRARY', issued_at: new Date().toISOString(), qr_payload: JSON.stringify({ type: 'spms-clearance-receipt', project_id: selected.id }) }; updateProject({ ...selected, status: 'cleared', receipt, cleared_at: new Date().toISOString() }); onToast('Preview clearance receipt issued.'); return; }
      setBusyAction('receipt'); const data = await invoke('project-workflow', { action: 'issue_receipt', project_id: selected.id }); updateProject({ ...data.project, receipt: data.receipt }); onToast(data.already_issued ? 'The existing clearance receipt was restored.' : 'Clearance receipt issued and project archived.');
    } catch (error) { onToast(error.message || 'Clearance receipt could not be issued.'); } finally { setBusyAction(''); }
  };
  const queueProjects = projects.filter(item => ['supervisor_approved', 'library_review'].includes(item.status));
  const dateMatches = item => queueDate === 'all' || (queueDate === 'today' ? new Date(item.updated_at || item.created_at).toDateString() === new Date().toDateString() : queueDate === 'week' ? Date.now() - new Date(item.updated_at || item.created_at).getTime() < 7 * 86400000 : true);
  const filteredQueueProjects = queueProjects.filter(item => (queueStatus === 'all' || item.status === queueStatus) && (queueDepartment === 'all' || item.department_id === queueDepartment) && (queueCourse === 'all' || item.course_id === queueCourse) && (queueDegree === 'all' || item.degree === queueDegree) && dateMatches(item) && [item.title, item.author, item.matric, item.profiles?.full_name, item.dept, item.department, item.course_name, item.degree].join(' ').toLowerCase().includes(queueQuery.trim().toLowerCase()));
  const catalogueProjects = projects.filter(item => ['published', 'cleared'].includes(item.status)).filter(item => (catalogStatus === 'all' || item.status === catalogStatus) && [item.title, item.dept, item.course_name, item.degree, item.shelf_number, item.doi].join(' ').toLowerCase().includes(catalogQuery.trim().toLowerCase()));
...
function LibraryWorkspacePages({ profile, preview, onToast, activeSection, navigate, projects, selected, setSelected, form, setForm, qrUrl, queueProjects, filteredQueueProjects, catalogueProjects, qrProjects, archiveProjects, departments, courses, queueQuery, setQueueQuery, queueStatus, setQueueStatus, queueDepartment, setQueueDepartment, queueCourse, setQueueCourse, queueDegree, setQueueDegree, queueDate, setQueueDate, catalogQuery, setCatalogQuery, catalogStatus, setCatalogStatus, openRecord, saveMetadata, verifyMetadata, generateQr, issueDoi, publish, issueClearanceReceipt, busyAction }) {
  ...
  return <Workspace role="library" title="Library verification desk" subtitle="Verify academic metadata, publish anonymized research, and complete final clearance." sidebar={sidebar} profile={profile} preview={preview} onToast={onToast}>
    <MetricCards items={[["Verification queue", queueProjects.length, FileCheck2], ["Published records", projects.filter(item => item.status === 'published').length, BookOpen], ["QR labels", qrProjects.length, QrCode], ["Cleared archive", archiveProjects.length, Archive]]} />
    {activeSection === 'library-queue' && <section className="surface workspace-section" id="library-queue"><SectionHeader eyebrow="Verification queue" title="Review incoming research" copy="Only supervisor-approved projects can enter library verification." action={<span className="tag"><LockKeyhole size={12} />Private PDFs</span>} /><div className="library-filter-grid"><SearchBox value={queueQuery} onChange={setQueueQuery} placeholder="Search title, student, matric, or course" /><select value={queueStatus} onChange={event => setQueueStatus(event.target.value)} aria-label="Library status filter"><option value="all">All statuses</option><option value="supervisor_approved">Needs verification</option><option value="library_review">Ready to publish</option></select><select value={queueDepartment} onChange={event => setQueueDepartment(event.target.value)} aria-label="Library department filter"><option value="all">All departments</option>{departments.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={queueCourse} onChange={event => setQueueCourse(event.target.value)} aria-label="Library course filter"><option value="all">All courses</option>{courses.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={queueDegree} onChange={event => setQueueDegree(event.target.value)} aria-label="Library degree filter"><option value="all">All degrees</option>{[...new Set(queueProjects.map(item => item.degree).filter(Boolean))].map(item => <option key={item} value={item}>{item}</option>)}</select><select value={queueDate} onChange={event => setQueueDate(event.target.value)} aria-label="Library date filter"><option value="all">Any date</option><option value="today">Updated today</option><option value="week">Updated this week</option></select></div>{filteredQueueProjects.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Research</th><th>Student</th><th>Academic mapping</th><th>Supervisor</th><th>Status</th><th>Action</th></tr></thead><tbody>{filteredQueueProjects.map(item => <tr key={item.id}><td><strong>{item.title}</strong><br /><span className="helper">{item.degree || 'Degree pending'}</span></td><td><div className="identity-cell"><ProfileAvatar name={item.author || item.profiles?.full_name} src={item.profiles?.avatar_url} /><div><strong>{item.author || item.profiles?.full_name || 'Student'}</strong><br /><span className="helper">{item.matric || 'Matric pending'}</span></div></div></td><td>{item.dept || 'Department pending'}<br /><span className="helper">{item.course_name || 'Course pending'}</span></td><td><span className="helper">{item.supervisor?.full_name || 'Supervisor not linked'}<br />{item.supervisor?.email || ''}</span></td><td><StatusChip status={item.status} /></td><td><button className="button button-primary button-small" type="button" onClick={() => openRecord(item)}>Open record</button></td></tr>)}</tbody></table></div> : <EmptyState icon={Library} title={queueProjects.length ? 'No matching queue records' : 'Queue is clear'} copy={queueProjects.length ? 'Adjust the filters to find another approved project.' : 'Supervisor-approved projects will arrive here for metadata verification.'} />}</section>}
...
function LibraryRecordModal({ selected, setSelected, form, setForm, qrUrl, departments, courses, selectedReadOnly, checklist, saveMetadata, verifyMetadata, generateQr, issueDoi, publish, issueClearanceReceipt, busyAction, onToast }) {
  ...
  const published = selected && ['published', 'cleared'].includes(selected.status);
  const verified = selected && ['library_review', 'published', 'cleared'].includes(selected.status);
  return <Modal open={Boolean(selected)} onClose={() => setSelected(null)} wide eyebrow={selectedReadOnly ? 'Published library record' : 'Library verification workspace'} title={selected?.title || 'Project record'} className="library-record-modal">
    {selected && <>
      <div className="library-record-heading"><div><p className="helper">Review the academic record, confirm the supervisor decision, and publish only anonymized metadata.</p><StatusChip status={selected.status} /></div><span className="tag"><LockKeyhole size={12} />Private record</span></div>
      <div className="library-record-grid">
        <div>
          <div className="library-person-grid">
            <div className="status-panel"><p className="eyebrow">Student</p><div className="identity-cell"><ProfileAvatar name={selected.author || selected.profiles?.full_name} src={selected.profiles?.avatar_url} size="avatar-medium" /><div><strong>{selected.author || selected.profiles?.full_name || 'Student'}</strong><span className="helper">{selected.matric || selected.profiles?.matric || 'Matric pending'}</span><ContactDetails person={selected.profiles} roleLabel="Student contact" /></div></div></div>
            <div className="status-panel"><p className="eyebrow">Supervisor approval</p><div className="identity-cell"><ProfileAvatar name={selected.supervisor?.full_name || 'Supervisor'} size="avatar-medium" /><div><strong>{selected.supervisor?.full_name || 'Supervisor not listed'}</strong><ContactDetails person={selected.supervisor} roleLabel="Supervisor contact" /></div></div><span className="helper">{selected.metadata_verified_at ? `Approved ${displayDate(selected.metadata_verified_at)}` : 'Supervisor approval confirmed'}</span></div>
          </div>
          <div className="library-metadata-form form-grid">
            <div className="field full"><label htmlFor="lib-title">Project title</label><input id="lib-title" value={form.title} disabled={selectedReadOnly} onChange={event => update('title', event.target.value)} /></div>
            <div className="field full"><label htmlFor="lib-abstract">Abstract</label><textarea id="lib-abstract" rows="5" value={form.abstract} disabled={selectedReadOnly} onChange={event => update('abstract', event.target.value)} /></div>
            <div className="field"><label htmlFor="lib-degree">Degree</label><input id="lib-degree" value={form.degree} disabled={selectedReadOnly} onChange={event => update('degree', event.target.value)} /></div>
            <div className="field"><label htmlFor="lib-department">Department</label><select id="lib-department" value={form.department_id} disabled={selectedReadOnly} onChange={event => update('department_id', event.target.value)}><option value="">Select department</option>{departments.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
            <div className="field"><label htmlFor="lib-course">Course</label><select id="lib-course" value={form.course_id} disabled={selectedReadOnly} onChange={event => update('course_id', event.target.value)}><option value="">Select course</option>{selectedCourses.map(item => <option key={item.id} value={item.id}>{item.name} {item.code ? `(${item.code})` : ''}</option>)}</select></div>
            <div className="field"><label htmlFor="lib-keywords">Keywords</label><input id="lib-keywords" value={form.keywords} disabled={selectedReadOnly} onChange={event => update('keywords', event.target.value)} placeholder="Separate keywords with commas" /></div>
            <div className="field"><label htmlFor="lib-shelf">Shelf number</label><input id="lib-shelf" value={form.shelf_number} disabled={selectedReadOnly} onChange={event => update('shelf_number', event.target.value)} placeholder="KASU-CS-001" /></div>
            <div className="field"><label htmlFor="lib-doi">DOI / catalogue identifier</label><input id="lib-doi" value={form.doi} disabled={selectedReadOnly} onChange={event => update('doi', event.target.value)} placeholder="Optional identifier" /></div>
            <div className="field full"><label htmlFor="lib-comment-input">Library verification note</label><textarea id="lib-comment-input" rows="3" value={form.library_note} disabled={selectedReadOnly} onChange={event => { update('library_note', event.target.value); update('comment', event.target.value); }} placeholder="Optional internal note for the audit trail" /></div>
          </div>
          <div className="library-pdf-panel"><div className="surface-head"><div><p className="eyebrow">Private thesis file</p><h3>Secure PDF preview</h3></div><span className="tag"><LockKeyhole size={12} />Never public</span></div>{selected.file_path ? <PdfPreview path={selected.file_path} /> : <div className="empty-state surface"><FileText size={22} color="#065f46" /><h3>{selected.isDemo ? 'Preview document record' : 'PDF unavailable'}</h3><p>{selected.isDemo ? 'A live record will open through a short-lived signed storage URL.' : 'The project does not have a readable PDF path.'}</p></div>}</div>
        </div>
        <aside>
          <div className="surface library-checklist"><p className="eyebrow">Publication checklist</p>{checklist.map(([label, complete]) => <div className={`library-check ${complete ? 'is-complete' : ''}`} key={label}><span>{complete ? <CheckCircle2 size={15} /> : <Circle size={15} />}{label}</span><small>{complete ? 'Ready' : 'Pending'}</small></div>)}</div>
          <div className="surface library-checklist"><p className="eyebrow">Project verification QR code</p>{qrUrl ? <img src={qrUrl} alt={`QR code for ${selected.title}`} style={{ width: 'min(210px, 100%)', display: 'block', margin: '0 auto' }} /> : <div className="empty-state"><QrCode size={24} color="#065f46" /><p>{published ? 'Preparing QR preview...' : 'A QR code is generated when the record is published.'}</p></div>}{qrUrl && <div className="card-actions"><a className="button button-ghost button-small" href={qrUrl} download={`${selected.shelf_number || selected.title}-qr.svg`}><Download size={14} />Download label</a></div>}</div>
...
function AdminLivePanel({ profile, session, initialSection, onToast }) {
  ...
  const nav = [[Archive, 'Dashboard', 'dashboard'], [Users, 'Students', 'students'], [UserCheck, 'Supervisor directory', 'supervisor-directory'], [Users, 'Student coverage', 'supervisor-coverage'], [FileCheck2, 'Assignment queue', 'supervisor-queue'], [Library, 'Departments', 'departments'], [FileText, 'Uploads', 'uploads'], [CircleDollarSign, 'Payments', 'payments'], [FileCheck2, 'Reports', 'reports'], [Settings2, 'Settings', 'settings']];
...
function AdminSupervisorQueue({ profile, onToast, page, preview = false }) {
  ...
  const createSupervisor = async event => {
    ...
    setSaving(true);
    try {
      if (preview) { setSupervisors(items => [...items, { id: `preview-supervisor-${Date.now()}`, full_name: supervisorForm.full_name, email: supervisorForm.email, phone: supervisorForm.phone, department: department?.name || 'Institution-wide' }]); setSupervisorForm({ full_name: '', email: '', phone: '', department_id: '', password: '' }); setCreateOpen(false); onToast('Preview supervisor account created.'); return; }
  ...
  const assignStudent = async (studentId, supervisorId) => {
    try {
      if (preview) { setStudents(items => items.map(item => item.id === studentId ? { ...item, supervisor_id: supervisorId } : item)); setQueue(items => items.filter(item => item.student_id !== studentId)); onToast('Preview student assigned to the supervisor.'); return; }
  ...
  const assignProject = async (projectId, supervisorId) => {
      try { if (preview) { setQueue(items => items.filter(item => item.id !== projectId)); onToast('Preview supervisor assignment completed.'); return; } await invoke('project-workflow', { action: 'assign_supervisor', project_id: projectId, supervisor_id: supervisorId }); setQueue(items => items.filter(item => item.id !== projectId)); onToast('Supervisor assigned and notification sent.'); }
    catch (error) { onToast(error.message); }
  };

  if (loading) return <PageSkeleton role="admin" />;
  const unassignedStudents = students.filter(student => !student.supervisor_id);
  if (page) return <SupervisorManagementPage page={page} supervisors={supervisors} students={students} queue={queue} departments={departments} unassignedStudents={unassignedStudents} createOpen={createOpen} setCreateOpen={setCreateOpen} assignOpen={assignOpen} setAssignOpen={setAssignOpen} supervisorForm={supervisorForm} setSupervisorForm={setSupervisorForm} saving={saving} createSupervisor={createSupervisor} assignStudent={assignStudent} assignProject={assignProject} />;
...
function SupervisorManagementPage({ page, supervisors, students, queue, departments, unassignedStudents, createOpen, setCreateOpen, assignOpen, setAssignOpen, supervisorForm, setSupervisorForm, saving, createSupervisor, assignStudent, assignProject }) {
  ...
  const isCoverage = page === 'coverage';
  return <section className="supervisor-management" id="admin-supervisors">
    {isDirectory && <section className="surface"><SectionHeader eyebrow="Supervisor management" title="Supervisor directory" copy="Create supervisors and review their official contact details." action={<button className="button button-primary button-small" type="button" onClick={() => setCreateOpen(true)}><UserPlus size={15} />Add supervisor</button>} />{supervisors.length ? <div className="supervisor-directory">{supervisors.map(supervisor => <div className="supervisor-directory-card" key={supervisor.id}><div className="identity-cell"><ProfileAvatar name={supervisor.full_name} src={supervisor.avatar_url} size="avatar-medium" /><div><h3>{supervisor.full_name || 'Unnamed supervisor'}</h3><ContactDetails person={supervisor} roleLabel={`${students.filter(student => student.supervisor_id === supervisor.id).length} assigned student${students.filter(student => student.supervisor_id === supervisor.id).length === 1 ? '' : 's'}`} /></div></div><button className="button button-ghost button-small" type="button" onClick={() => setAssignOpen(true)}><UserCheck size={14} />Assign student</button></div>)}</div> : <EmptyState icon={UserPlus} title="No supervisors yet" copy="Create the first supervisor account to start routing student projects." />}</section>}
    {isCoverage && <section className="surface"><SectionHeader eyebrow="Coverage control" title="Student supervisor coverage" copy="Every student is shown here so no one is left without an assigned supervisor." action={<span className={unassignedStudents.length ? 'tag tag-warning' : 'tag'}>{unassignedStudents.length ? `${unassignedStudents.length} need assignment` : 'All students covered'}</span>} />{students.length ? <div className="contact-directory">{students.map(student => <StudentAssignmentRow key={student.id} student={student} supervisors={supervisors} onAssign={assignStudent} />)}</div> : <EmptyState icon={Users} title="No student profiles found" copy="Registered student accounts will appear here." />}</section>}
    {!isDirectory && !isCoverage && <section className="surface"><SectionHeader eyebrow="Workflow exceptions" title="Unassigned Review Queue" copy="Projects without a supervisor are held here until an administrator resolves the assignment." action={<span className="tag"><LockKeyhole size={12} />Protected action</span>} />{queue.length ? <div className="queue-list">{queue.map(item => <AssignmentRow item={item} supervisors={supervisors} onAssign={assignProject} key={item.id} />)}</div> : <EmptyState icon={UserCheck} title="Assignment queue is clear" copy="Every active project currently has a supervisor." />}</section>}
    <Modal open={createOpen} onClose={() => setCreateOpen(false)} eyebrow="Supervisor management" title="Add new supervisor"><form className="form-grid" onSubmit={createSupervisor}><div className="field full"><label htmlFor="new-supervisor-name">Full name</label><input id="new-supervisor-name" required value={supervisorForm.full_name} onChange={event => setSupervisorForm({ ...supervisorForm, full_name: event.target.value })} placeholder="Dr. Amina Yusuf" /></div><div className="field"><label htmlFor="new-supervisor-email">Email address</label><input id="new-supervisor-email" type="email" required value={supervisorForm.email} onChange={event => setSupervisorForm({ ...supervisorForm, email: event.target.value })} placeholder="supervisor@kasu.edu.ng" /></div><div className="field"><label htmlFor="new-supervisor-phone">Phone number</label><input id="new-supervisor-phone" value={supervisorForm.phone} onChange={event => setSupervisorForm({ ...supervisorForm, phone: event.target.value })} placeholder="0803 000 0000" /></div><div className="field"><label htmlFor="new-supervisor-department">Department</label><select id="new-supervisor-department" required value={supervisorForm.department_id} onChange={event => setSupervisorForm({ ...supervisorForm, department_id: event.target.value })}><option value="">Select department</option>{departments.map(item => <option value={item.id} key={item.id}>{item.name}</option>)}</select></div><PasswordField id="new-supervisor-password" label="Temporary password" value={supervisorForm.password} onChange={event => setSupervisorForm({ ...supervisorForm, password: event.target.value })} minLength="6" autoComplete="new-password" placeholder="At least 6 characters" /><p className="helper full">The account is confirmed immediately. Share the temporary password securely and ask the supervisor to change it after first login.</p><div className="modal-actions full"><button className="button button-ghost" type="button" onClick={() => setCreateOpen(false)}>Cancel</button><button className="button button-primary" disabled={saving}>{saving ? 'Creating...' : 'Create supervisor'}</button></div></form></Modal>
    <Modal open={assignOpen} onClose={() => setAssignOpen(false)} eyebrow="Coverage control" title="Assign a student" wide><p className="helper">Choose a supervisor directly from the student coverage list below.</p><div className="contact-directory" style={{ marginTop: '1rem' }}>{students.map(student => <StudentAssignmentRow key={`modal-${student.id}`} student={student} supervisors={supervisors} onAssign={(studentId, supervisorId) => { assignStudent(studentId, supervisorId); setAssignOpen(false); }} />)}</div></Modal>
  </section>;
}
...
function AdminPreviewWorkspace({ section, setSection }) {
  const nav = [[Archive, 'Dashboard', 'dashboard'], [Users, 'Students', 'students'], [UserCheck, 'Supervisor directory', 'supervisor-directory'], [Users, 'Student coverage', 'supervisor-coverage'], [FileCheck2, 'Assignment queue', 'supervisor-queue'], [Library, 'Departments', 'departments'], [FileText, 'Uploads', 'uploads'], [CircleDollarSign, 'Payments', 'payments'], [FileCheck2, 'Reports', 'reports'], [Settings2, 'Settings', 'settings']];
  return <Workspace role="admin" title="Analytics hub" subtitle="Live operations for academic clearance, finance, and institutional governance." sidebar={nav.map(([Icon, label, id]) => [Icon, label, section === id, () => setSection(id)])}><MetricCards items={[["Total students", demoStats.students, Users], ["Total revenue", '₦8.43m', CircleDollarSign], ["Total uploads", demoStats.submitted, FileText], ["Pending approvals", '216', FileCheck2]]} />{section === 'dashboard' && <div className="workspace-grid"><AnalyticsCard title="Workflow funnel" copy="Live status distribution across the clearance pipeline." values={[["Submitted",920],["Supervisor review",780],["Library review",650],["Cleared",612]]} /><AnalyticsCard title="Revenue split" copy="Institution and provider shares reconcile against Paystack records." values={[["Institution share",80],["SPMS provider share",20],["Legacy / unallocated",4]]} accent /></div>}{section === 'students' && <section className="surface"><SectionHeader eyebrow="Student directory" title="Registered students" copy="Searchable student records connected to project ownership and department mapping." /><DataTable columns={['Name', 'Matric', 'Email', 'Department']} rows={[["Musa Abdullahi", 'KASU/SCI/20/123', 'student@kasu.edu.ng', 'Computer Science']]} empty="No student profiles found." /></section>}{['supervisor-directory', 'supervisor-coverage', 'supervisor-queue'].includes(section) && <AdminSupervisorQueue profile={{ id: 'preview-admin', role: 'admin', full_name: 'SPMS Administrator' }} onToast={() => {}} page={section.replace('supervisor-', '')} preview />}{section === 'reports' && <section className="surface" id="admin-reports"><SectionHeader eyebrow="Reporting" title="Scheduled reporting controls" copy="Export operational, lifecycle, financial, and archive evidence for registry review." /><div className="status-panel"><h3>Project lifecycle</h3><p className="helper">Monthly workflow export with audit events and status changes.</p><span className="tag">project-lifecycle-preview.csv</span></div></section>}{!['dashboard', 'students', 'supervisor-directory', 'supervisor-coverage', 'supervisor-queue', 'reports'].includes(section) && <section className="surface"><EmptyState icon={Settings2} title={`${section[0].toUpperCase()}${section.slice(1)} operations`} copy="This workspace is connected to the SPMS role shell and ready for its data view." /></section>}</Workspace>;
}

function AdminWorkspace({ profile, session, preview, onToast }) {
  const [section, setSection] = useState(previewAction === 'open_reports' ? 'reports' : previewAction === 'open_assignments' ? 'supervisor-queue' : 'dashboard');
  ...
  return <AdminPreviewWorkspace section={section} setSection={setSection} />;
}
