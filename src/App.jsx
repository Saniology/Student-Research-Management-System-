...
function Landing({ tenant, session, onLogin, onWorkspace, onDownload, configError }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    const catalogQuery = supabase.from('public_catalog').select('id, title, department_name, degree, abstract, published_at, project_id, institution_id').order('published_at', { ascending: false }).limit(12);
    ...
    Promise.all([catalogQuery, publishedCountQuery, departmentQuery]).then(([catalogResult, publishedResult, departmentResult]) => {
      ...
      setProjects((catalogResult.data || []).map(item => ({ ...item, author: 'Anonymized researcher', dept: item.department_name, status: 'published' })));
