...
function StudentWorkspacePages({ profile, session, preview, onToast, onProfileUpdate, activeSection, navigate, project, payment, receipt, supervisor, clearanceFee, maxPdfBytes, submitting, pendingVerification, receiptQrUrl, form, setForm, file, setFile, submit, retryPendingVerification, generateReceipt }) {
  ...
  const sidebar = [
    ...
    [CircleDollarSign, 'Payments', activeSection === 'student-payments', () => navigate('student-payments')],
    [ShieldCheck, 'Receipt', activeSection === 'student-receipt', () => navigate('student-receipt')],
    [Settings2, 'Profile', activeSection === 'student-profile', () => navigate('student-profile')],
  ];
  ...
  return <Workspace role="student" title="Your clearance workspace" subtitle="A focused path for submission, review progress, payment evidence, and digital clearance." sidebar={sidebar}>
    ...
    {activeSection === 'student-profile' && <div className="workspace-section" id="student-profile"><ProfileContactEditor profile={profile} session={session} preview={preview} onProfileUpdate={onProfileUpdate} onToast={onToast} /><SupervisorContactCard supervisor={supervisor} heading="Assigned supervisor" /></div>}
...
function SupervisorWorkspacePage({ profile, activeSection, navigate, projects, students, reviewProjects, historyProjects, canReview, openReview, selected, setSelected, comment, setComment, review }) {
  const sidebar = [
    ...
    [Settings2, 'Supervisor profile', activeSection === 'teacher-profile', () => navigate('teacher-profile')],
  ];

  return <Workspace role="teacher" title="Supervisor review queue" subtitle="A focused workspace for assigned research, student support, and auditable decisions." sidebar={sidebar}>
    {activeSection === 'teacher-overview' && <div className="workspace-section" id="teacher-overview">
      ...
      <div className="workspace-grid supervisor-overview-grid">
        ...
        <section className="surface supervisor-focus-panel"><div className="surface-head"><div><p className="eyebrow">Academic support</p><h2>Supervisor profile</h2><p>Your official contact details are available to assigned students.</p></div><UserCheck size={18} color="#065f46" /></div><div className="supervisor-profile-compact"><ProfileAvatar name={profile?.full_name} src={profile?.avatar_url} size="avatar-medium" /><div><h3>{profile?.full_name || 'Supervisor'}</h3><ContactDetails person={profile} roleLabel="Supervisor account" /></div></div><button className="button button-ghost button-small supervisor-inline-action" type="button" onClick={() => navigate('teacher-profile')}><Settings2 size={14} />Manage profile</button></section>
      </div>
    </div>}
    ...
    {activeSection === 'teacher-profile' && <div className="workspace-section" id="teacher-profile"><SupervisorContactCard supervisor={profile} heading="Supervisor profile" missingCopy="Your supervisor profile is managed by the institution." /><section className="surface supervisor-preferences"><div className="surface-head"><div><p className="eyebrow">Workspace settings</p><h2>Review preferences</h2><p>Your department and secure preview settings are institution-managed.</p></div><Settings2 size={18} color="#065f46" /></div><div className="queue-list"><div className="queue-item"><div><h3>Department</h3><p>{profile?.department || 'Computer Science'}</p></div><span className="tag">Institution managed</span></div><div className="queue-item"><div><h3>Private PDF previews</h3><p>Signed links expire automatically after five minutes.</p></div><LockKeyhole size={17} color="#065f46" /></div></div></section></div>}
    <Modal open={Boolean(selected)} onClose={() => setSelected(null)} eyebrow="Supervisor review" title={selected?.title || ''} wide><div className="modal-body-grid"><div><div className="status-panel"><div className="identity-cell"><ProfileAvatar name={selected?.author} src={selected?.profiles?.avatar_url} size="avatar-medium" /><div><h3>{selected?.author || 'Student'}</h3><p className="helper">{selected?.matric} - {selected?.dept || 'Computer Science'} - {selected?.degree}</p><ContactDetails person={selected?.profiles} roleLabel="Student contact" /></div></div><StatusChip status={selected?.status} /></div><p className="helper" style={{ marginTop: '1rem', lineHeight: 1.65 }}>{selected?.abstract}</p><div className="field" style={{ marginTop: '1rem' }}><label htmlFor="modal-review-comment">Decision comment</label><textarea id="modal-review-comment" value={comment} onChange={e => setComment(e.target.value)} placeholder="Add an approval note or explain the revision needed." /></div></div><div><p className="eyebrow">Private PDF preview</p>{selected?.isDemo ? <div className="empty-state surface"><FileText size={23} color="#065f46" /><h3>Preview unavailable in demo mode</h3><p>Real projects open through a five-minute signed storage URL.</p></div> : <PdfPreview path={selected?.file_path} />}</div></div><div className="modal-actions"><button className="button button-ghost" disabled={!canReview(selected?.status)} onClick={() => review('request_revision')}><PencilLine size={15} />Request revision</button><button className="button button-primary" disabled={!canReview(selected?.status)} onClick={() => review('approve')}><Check size={15} />Approve Project</button></div></Modal>
  </Workspace>;
}
