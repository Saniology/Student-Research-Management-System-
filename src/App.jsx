...
function TeacherWorkspace({ profile, session, preview, onToast }) {
  ...
  useEffect(() => {
    ...
    supabase.from('projects').select('*, profiles!projects_student_id_fkey(full_name,matric,avatar_url)').eq('institution_id', profile.institution_id).eq('supervisor_id', session.user.id).order('created_at', { ascending: false }).then(({ data, error }) => { if (error) throw error; setProjects((data || []).map(item => ({ ...item, author: item.profiles?.full_name, matric: item.profiles?.matric, dept: profile?.department, isDemo: false }))); }).catch(error => { setProjects([]); onToast(error.message || 'Supervisor queue could not be loaded.'); }).finally(() => setLoading(false));
    return undefined;
  }, [onToast, preview, session, profile]);
...
function LibraryWorkspace({ profile, session, preview, onToast }) {
  ...
  return <Workspace role="library" title="Library verification desk" subtitle="Verify metadata, assign shelf records, and publish approved research." sidebar={[[Library, 'Verification queue', activeSection === 'library-queue', () => navigate('library-queue')], [BookOpen, 'Public catalogue', activeSection === 'library-catalogue', () => navigate('library-catalogue')], [QrCode, 'QR labels', activeSection === 'library-qr', () => navigate('library-qr')], [Archive, 'Archive', activeSection === 'library-archive', () => navigate('library-archive')]]}>
    ...
    <section className="surface workspace-section" id="library-queue"><div className="surface-head"><div><h2>Verification queue</h2><p>Only supervisor-approved projects can enter the public catalogue.</p></div><span className="tag"><QrCode size={12} />QR ready</span></div>{queueProjects.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Research</th><th>Author</th><th>Department</th><th>Status</th><th>Action</th></tr></thead><tbody>{queueProjects.map(item => <tr key={item.id}><td><strong>{item.title}</strong><br /><span className="helper">{item.degree}</span></td><td>{item.author || item.profiles?.full_name || 'Student'}</td><td>{item.dept || 'Computer Science'}</td><td><StatusChip status={item.status} /></td><td><button className="button button-primary button-small" type="button" onClick={() => openRecord(item)}>Open record</button></td></tr>)}</tbody></table></div> : <EmptyState icon={Library} title="Queue is clear" copy="Supervisor-approved projects will arrive here for metadata verification." />}</section>
...
function AdminSupervisorQueue({ profile, onToast }) { const [queue, setQueue] = useState([]); const [supervisors, setSupervisors] = useState([]); const [loading, setLoading] = useState(true); useEffect(() => { if (!profile?.institution_id) { setQueue([]); setSupervisors([]); setLoading(false); onToast('Your admin account is not linked to an institution.'); return undefined; } const projectsQuery = supabase.from('projects').select('id,title,status,profiles!projects_student_id_fkey(full_name,matric,department)').eq('institution_id', profile.institution_id).is('supervisor_id', null).in('status', ['submitted', 'supervisor_review']); const supervisorsQuery = supabase.from('profiles').select('id,full_name,department').eq('institution_id', profile.institution_id).eq('role', 'teacher'); Promise.all([projectsQuery, supervisorsQuery]).then(([projectsResult, supervisorsResult]) => { if (projectsResult.error) throw projectsResult.error; if (supervisorsResult.error) throw supervisorsResult.error; setQueue((projectsResult.data || []).map(item => ({ ...item, author: item.profiles?.full_name, matric: item.profiles?.matric, dept: item.profiles?.department }))); setSupervisors(supervisorsResult.data || []); }).catch(error => { setQueue([]); setSupervisors([]); onToast(error.message || 'Supervisor queue could not be loaded.'); }).finally(() => setLoading(false)); return undefined; }, [profile, onToast]); const assign = async (projectId, supervisorId) => { try { await invoke('project-workflow', { action: 'assign_supervisor', project_id: projectId, supervisor_id: supervisorId }); setQueue(items => items.filter(item => item.id !== projectId)); onToast('Supervisor assigned and notification sent.'); } catch (error) { onToast(error.message); } }; if (loading) return <PageSkeleton role="admin" />; return <section className="surface" id="admin-supervisors"><SectionHeader eyebrow="Supervisor management" title="Unassigned Review Queue" copy="Resolve submissions that could not be automatically matched to an eligible supervisor." action={<span className="tag"><UserCheck size={12} />Protected action</span>} />{queue.length ? <div className="queue-list">{queue.map(item => <AssignmentRow item={item} supervisors={supervisors} onAssign={assign} key={item.id} />)}</div> : <EmptyState icon={UserCheck} title="Assignment queue is clear" copy="Every eligible submission currently has a supervisor." />}</section>; }
