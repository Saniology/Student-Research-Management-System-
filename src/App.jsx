...
function AdminLivePanel({ profile, session, initialSection, onToast }) {
  ...
  const loadAdminData = useCallback(async () => {
    ...
    const receiptQuery = supabase.from('clearance_receipts').select('id,project_id,student_id,verification_code,qr_payload,issued_at,projects(title)').eq('projects.institution_id', activeInstitutionId).order('issued_at', { ascending: false }).limit(100);
