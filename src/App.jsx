...
import { issueReceipt, lookupVerification, retryPaymentVerification, runDueReports, runScheduledReport, workflowActions } from './lib/contracts';
...
function AdminLivePanel({ profile, session, initialSection, onToast }) {
  ...
  const loadAdminData = useCallback(async () => {
    ...
    const [overviewResult, studentsResult, projectResult, paymentResult, departmentResult, facultyResult, collegeResult, institutionResult, configResult, schedulesResult, reportResult] = await Promise.all([
      ...
      supabase.from('profiles').select('id,full_name,email,matric,department,department_id,created_at').eq('role', 'student').order('created_at', { ascending: false }).limit(100),
      supabase.from('projects').select('id,title,status,degree,created_at,updated_at,profiles!projects_student_id_fkey(full_name,matric),departments(name)').order('created_at', { ascending: false }).limit(100),
      ...
      supabase.from('departments').select('id,name,code,faculty_id,faculties(name)').order('name'),
      supabase.from('faculties').select('id,name,college_id,colleges(name)').order('name'),
    ...
    setGeneratedReports(reportResult.data || []);
    setLoading(false);
  }, [profile, session]);
  ...
  return <Workspace role="admin" title="Analytics hub" subtitle="Live operations for academic clearance, finance, and institutional governance." sidebar={nav.map(([Icon, label, id]) => [Icon, label, section === id, () => setSection(id)])}>
    ...
    {section === 'supervisors' && <AdminSupervisorQueue session={session} onToast={onToast} />}
