...
function LibraryWorkspace({ profile, session, preview, onToast }) {
  ...
  const saveMetadata = async silent => {
    ...
    if (!libraryProjectMetadataComplete(selected, form)) throw new Error('Title, abstract, degree, department, course, and a PDF are required. Select the course before continuing.');
      ...
      <div className="library-record-grid">
        <div>
          ...
          <div className="library-metadata-form form-grid">
            ...
            <div className="field"><label htmlFor="lib-department">Department</label><select id="lib-department" value={form.department_id} disabled={selectedReadOnly} onChange={event => { const departmentId = event.target.value; const departmentCourses = courses.filter(course => course.department_id === departmentId); update('department_id', departmentId); update('course_id', departmentCourses.length === 1 ? departmentCourses[0].id : ''); }}><option value="">Select department</option>{departments.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
